...
class CLIDiarizer:
    def __init__(self, device_idx, threshold=0.5, expected_speakers=2, min_speech_duration=0.75, db_path="models/voice_profiles.db", lc_port=47832, dump_wav=False, debug=False):
        ...
        self.chunk_size = 512
        # Hard cap ~1.2s so one embedding rarely mixes two dialogue turns
        # (log café: 2.5–3.0s segments merged customer+barista into one vector).
        self.max_speech_duration = 1.2
        # End segment after this much non-speech (ms). Tight = split turns sooner.
        self.min_silence_ms = 200
    ...
    def vad_processing_loop(self):
        ...
        while self.is_running or not self.chunk_queue.empty():
            ...
            min_silence_chunks = max(1, int(self.min_silence_ms / (chunk_size / sr * 1000)))
            ...
            if is_speech:
                ...
                if current_duration >= self.max_speech_duration:
                    # Force-cut long speech; continue next segment immediately so
                    # continuous talk becomes multiple short embeddings (less mix).
    ...
    def run_clustering(self):
        ...
        segments = []
        # Include from first assignment so STT can resolve speaker labels immediately.
        # Recognition (NavPane list) waits until seg_count >= 2 to avoid Spk-03 flash
        # from one-shot noise / label churn.
        TIMELINE_MIN_SEGS = 1
        RECOGNITION_MIN_SEGS = 2
        ...
        if segments:
            ...
            # Announce confirmed session UIDs once (need >=2 segments, or DB match)
            if not hasattr(self, "_announced_uids"):
                self._announced_uids = set()
            for uid, pdata in self.speaker_profiles_data.items():
                if uid in self._announced_uids:
                    continue
                seg_count = pdata.get('count', 0)
                is_recognized = uid in self.uid_to_profile_id
                if seg_count < RECOGNITION_MIN_SEGS and not is_recognized:
                    continue
                self._announced_uids.add(uid)
                display = uid
                if uid in self.uid_to_profile_id:
                    pid = self.uid_to_profile_id[uid]
                    display = self.db.get_metadata(pid, "display_name") or pid[:8]
                self.emit({
                    "type": "recognition",
                    "uid": uid,
                    "profile_id": self.uid_to_profile_id.get(uid, uid),
                    "display_name": display,
                    "dist": 0.0,
                    "method": "session-cluster"
                })
...
def find_wasapi_loopback_device(p):
    """
    Pick WASAPI loopback matching the *current default Windows output*
    (speakers jack, headphones, HDMI, USB DAC, etc.) so diarizer hears the
    same system audio Live Captions captions — not the microphone.
    """
    default_out_name = ""
    try:
        default_out_name = (p.get_default_output_device_info().get("name") or "").strip()
    except Exception:
        pass

    # Normalize "Headphones (Realtek(R) Audio)" → "headphones"
    def _base(n: str) -> str:
        return (n or "").split("(")[0].strip().lower()

    default_base = _base(default_out_name)

    candidates = []
    # Prefer pyaudiowpatch loopback generator when available
    try:
    ...
    if not loopbacks:
        for idx in range(p.get_device_count()):
            ...
            is_lb = bool(dev.get("isLoopbackDevice", False)) or ("[Loopback]" in name)
            if is_lb:
                loopbacks.append({**dev, "index": idx})
    ...
    for dev in loopbacks:
        ...
        if idx < 0:
            continue
        score = 1
        lb_base = _base(name.replace("[Loopback]", ""))
        # Strongly prefer loopback of the active default output device
        if default_base and (default_base in lb_base or lb_base in default_base):
            score += 100
        if default_out_name and default_out_name.split("(")[0].strip() in name:
            score += 50
        lower = name.lower()
        if "speaker" in lower:
            score += 5
        if "headphone" in lower or "headset" in lower:
            score += 5
        candidates.append((score, idx, name, default_out_name))

    if not candidates:
        return None, None
    candidates.sort(key=lambda x: (-x[0], x[1]))
    best = candidates[0]
    return best[1], f"{best[2]} (default_out={default_out_name or 'unknown'})"
...
def main():
            ...
            for idx in range(device_count):
                ...
                if dev['maxInputChannels'] > 0:
                    devices.append({
                        ...
                        "is_loopback": dev.get('isLoopbackDevice', False) or ("[Loopback]" in (dev.get("name") or ""))
                    })
        except Exception:
            pass
        finally:
    ...
    parser.add_argument("--device", type=int, default=-1,
                        help="Audio input device index; -1 = auto WASAPI loopback (system audio)")
    ...
    if device_idx < 0:
        ...
        try:
            device_idx, dev_name = find_wasapi_loopback_device(p_tmp)
        finally:
            p_tmp.terminate()
        if device_idx is None:
            print("[FATAL] No WASAPI loopback device found; pass --device <index>", file=sys.stderr)
            sys.stdout.write(json.dumps({"type": "error", "message": "No WASAPI loopback device found"}) + "\n")
            sys.stdout.flush()
            sys.exit(1)
        print(f"[AUDIO] Auto-selected loopback device {device_idx}: {dev_name}", file=sys.stderr)
    else:
        print(f"[AUDIO] Using device index {device_idx}", file=sys.stderr)
